...
function Poster({data}:{data:IGetSlideData[]}) {
  return (
    <div className="w-full flex flex-col xl:flex-row gap-12 justify-between items-center">
        {(data).map((sld, index)=> 
            sld.id == "01" || sld.id == "03"?
                <PosterBox keys={index} poster={sld} /> 
            : null 
        )}   
    </div>
  )
...
function PosterBox({poster,keys}:{poster:IGetSlideData,keys:number}){
    return(
        <div key={keys} className="rounded-2xl overflow-hidden w-full">
