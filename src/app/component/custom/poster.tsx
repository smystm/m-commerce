...
import { IGetSlideData } from '@/types/index';
...
function PosterBox({key, poster}:{poster:IGetSlideData,key:number}){
    return(
        <div key={key} className="rounded-2xl overflow-hidden w-full">
            <div style={{backgroundImage:`url(${poster.image_url})`}} className={`w-full h-45 sm:h-55 md:h-75 lg:h-90 xl:h-70 2xl:h-85 rounded-xs md:rounded-none flex flex-col justify-center items-start text-white 2xl:px-10 bg-cover bg-center bg-no-repeat bg-gradient-to-r from-[#ffffff09] to-[#00000077] `}>
